...
adduser --gecos $USER_PI --disabled-password $USER_PI
echo "$USER_PI:${ROOT_PASSWORD}" | chpasswd
echo spi i2c gpio | xargs -n 1 groupadd -r
usermod -a -G adm,dialout,sudo,audio,video,plugdev,users,netdev,input,spi,gpio,i2c,sudo $USER_PI
_EOF
...
if [[ "$OS" == "debian" && "$VARIANT" == "full" ]]; then
  ...
  cmake -Wno-deprecated -DCMAKE_TOOLCHAIN_FILE="makefiles/cmake/toolchains/${LIB_ARCH}.cmake" \
